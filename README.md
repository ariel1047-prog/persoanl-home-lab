...
| Device Name | CPU | GPU | RAM | Storage | Primary Role |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **BEAST** | AMD Ryzen 3 3300X <br>*(4C / 8T)* | NVIDIA RTX 4060 Ti <br>*(8GB)* | 32 GB DDR4 <br>@ 3600 MHz | 512GB M.2 NVMe SSD <br>1TB M.2 NVMe SSD | Core Network Bus, Pop!_OS Gateway, <br>AAA Captive Portal, Workstation & Gaming PC |
| **AsusPro E830** | Intel Core i5-4770 vPro <br>*(4C / 8T)* | Integrated Intel HD | 16 GB DDR3 SODIMM <br>@ 1600 MHz | 1 TB SATA SSD | NEXUS_MID: Primary Proxmox VE <br>Virtualization Host |
| **NEXUS_COMPUTE** <br>*(Asuspro Headless Laptop)* | Intel Core i3 <br>*(10th Gen)* | Integrated Intel HD | 12 GB DDR4 SODIMM <br>@ 3200 MHz | 512GB M.2 NVMe SSD <br>1TB SATA HDD *(Planned)* | Secondary Proxmox VE Node <br>*(Dedicated Compute Node)* |
| **ZOMBI_PC** | Intel Core CPU <br>*(Legacy)* | *GPU Selection TBD* <br>*(R9 390 / GTX 560 / RX 550)* | 16 GB DDR3 <br>*(4x4GB @ 1600 MHz)* | *TBD (Storage drive selection in progress)* | Future NAS & Legacy OS <br>Sandbox Environment |
...
* **Client Mobile:** Samsung Galaxy S23 Ultra
...
* **Vaultwarden:** Self-hosted password management (secured via DuckDNS).
...
### 3. FRONTEND (User Access & Deployment Status)
...
* **ZOMBI_PC (Isolated VM Environment):**
    * 🟡 **Windows 7 Legacy OS Sandbox** — *Planned Environment with Direct GPU Passthrough optimized for retro gaming and hardware benchmarking (e.g., executing Crysis).*
...
* **Decoupled Password Vault Architecture:** * *Compute Layer:* The Vaultwarden application instance runs as a dedicated VM on the **Asuspro Headless Laptop** for optimized processing and isolation.
    * *Storage & Persistence Layer:* The encrypted database and runtime data are directly mapped and backed up onto the **ZOMBI_NAS** via secure network shares (NFS/SMB), decoupling the application from the underlying storage.
